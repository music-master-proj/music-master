...
const authenticate = async (req, res, next) => {
  try {
    // checking for token inside header bearer 
    const token = await req.header("authorization")?.split(" ")[1];

    if (!token) return res.status(401).json({ message: "No token, Authentication Denied!!!" }) // check if token is available or not

    // Verify token
    const decoded = jwt.verify(token, process.env.ACCESS_TOKEN_SECRET); req.user = decoded.user;
    return next();
  } catch (error) {
