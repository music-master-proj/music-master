...
function Profile() {
    ...
    return ( <
        > {!authData ? < Redirect to = '/login' / > :
            ...
            footer = { authData.user.role === 0 ? false : true } > {
                authData.user.role === 1 ? ( <
                    >
                    <
                    h2 className = ' text-uppercase fw-bolder mx-5'
                    style = {
                        { color: 'var(--theme-navbarColor)' } } > Hello, { authData.user.name } < /h2> {
                        loading ? < Loader / > : ( <
                            >
                            <
                            Recommendations artists = { artistSongs }
                            tracks = { trackSongs }
                            /> <
                            AllPlaylists playlists = { playlists }
                            user = { authData.user }
                            playlistHandler = { fetchPlaylists }
                            /> <
                            PlaylistForm user = { authData.user }
                            allSongs = { allSongsCombined }
                            /> <
                            />
                        )
                    } <
